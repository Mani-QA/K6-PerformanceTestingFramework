...
// Define target APIs tested based on env and dynamic test run metadata
let targetApis = [];
if (summaryData.metadata && Array.isArray(summaryData.metadata.targetApis)) {
  targetApis = summaryData.metadata.targetApis.map(api => {
    const parts = api.trim().split(/\s+/);
    const method = parts[0] || 'GET';
    const path = parts[1] || '';
    return `${method} ${config.baseUrl}${path.startsWith('/') ? path : '/' + path}`;
  });
} else {
  // Dynamic fallback: parse checks in summary.json to detect HTTP methods and match them to users endpoints
  const checks = (summaryData.root_group && summaryData.root_group.checks) || [];
  const detectedMethods = new Set();
  checks.forEach(check => {
    const match = check.name.match(/^(GET|POST|PATCH|PUT|DELETE)/i);
    if (match) {
  ...
  if (detectedMethods.size > 0) {
    detectedMethods.forEach(method => {
      if (method === 'POST') targetApis.push(`POST ${config.baseUrl}/users`);
      else if (method === 'GET') targetApis.push(`GET ${config.baseUrl}/users/:id`);
      else if (method === 'PATCH') targetApis.push(`PATCH ${config.baseUrl}/users/:id`);
    });
  } else {
    // Default fallback
    targetApis = [
...
if (fs.existsSync(runsJsonPath)) {
  try {
    ...
    // Migrate/update historical runs to correct target APIs instead of hardcoded placeholder `/get` and `/post`
    history = history.map(run => {
      if (run.targetApis && run.targetApis.some(api => api.endsWith('/get') || api.endsWith('/post'))) {
        let baseUrl = 'https://k6demo.manig.dev';
        const existingGet = run.targetApis.find(api => api.startsWith('GET '));
        if (existingGet) {
          baseUrl = existingGet.substring(4).replace(/\/get$/, '');
        }
        run.targetApis = [
          `POST ${baseUrl}/users`,
  ...
  <script>
    ...
    function selectRun(runId) {
      ...
      run.targetApis.forEach(api => {
        let method = 'GET';
        if (api.startsWith('POST')) method = 'POST';
        else if (api.startsWith('PATCH')) method = 'PATCH';
        
        let badgeClass = 'method-get';
        if (method === 'POST') badgeClass = 'method-post';
        else if (method === 'PATCH') badgeClass = 'method-patch';
        
        const badge = \`<span class="method-badge \${badgeClass}">\${method}</span>\`;
        const url = api.replace(/^(GET|POST|PATCH)\\s+/, '');
        apiItems += \`
          <li class="api-item">
            \${badge}
            <span>\${url}</span>
